...
const errorLogging = require('../lib/errorHandler.js')
errorLogging.loggingSetting = 'simple'
  ...
  let mapping = function(array, callback){
    ...
    if (isArgsArray) {
      try {
        ...
        new errorLogging(e, 'trying to do a call on an array method')
      }
    } else {
      try {
        doit = Array.prototype[method].call(array, params);
...
function arrayBind(method, isArgsArray){
  let binding = function(array, callback){
    ...
    try {
      ...
      new errorLogging(e, 'trying to bind to an array method')
    }
  };
  return binding;
}
...
function arrayApply(method, isArgsArray){
  let applying = function(array, callback){
    ...
    try {
      ...
      new errorLogging(e, 'trying to use apply on an array method')
    }
  };
  return applying;
}
