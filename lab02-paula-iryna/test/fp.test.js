...
describe('mapCall', function() {
  ...
  it ('should return an error if "numbers" is not an array', function(){
    let result = fp.mapCall(null, (a=>a+2));
    expect(result).toBe('first parameter must be array');
  });
...
describe('mapApply', function(){
  ...
  it ('should return an error if callback is not an array', function(){
    let result = fp.mapApply(null, (a=>a+2));
    expect(result).toBe('callback parameter must be an array');
  });
